...
export class BestDmgAI {
    ...
    public Record(fightReprot: FightReport) {
    ...
    public reportOnHolding() {
        return;
        console.log(`${this._cur + 1}/${this._allPossible.length}`);
    }
    public reportSumamry() {    
        ...
        for(var i = 0;i < 5;i++){
            console.log("TOP " + i);
            console.log(...this._FetchPossibleCardInfo(tmp[i].index).map(info => info.name+info.level));
            console.log(...tmp[i].fr.heRoundHp);
